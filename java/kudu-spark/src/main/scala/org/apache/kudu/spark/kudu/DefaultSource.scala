...
class DefaultSource extends RelationProvider with CreatableRelationProvider
  ...
  BaseRelation = {
    ...
    val batchSizeOption = parameters.get("kudu.batch.size")
    val batchSize = batchSizeOption.getOrElse(1024 * 1024 * 20).toString.toInt
    new KuduRelation(tableName, kuduMaster, operationType, None, batchSize, alias)(sqlContext)
  }
  ...
  override def createRelation(sqlContext: SQLContext, parameters: Map[String, String],
    ...
    val batchSizeOption = parameters.get("kudu.batch.size")
    val batchSize = batchSizeOption.getOrElse(1024 * 1024 * 20).toString.toInt
    new KuduRelation(tableName, kuduMaster, operationType, Some(schema), batchSize, alias)(sqlContext)
  }
...
class KuduRelation(private val tableName: String,
  ...
  override def schema: StructType = {
    val fields =
      userSchema match {
        case Some(x) => x.fields.map(uf => table.getSchema.getColumn(uf.name))
  ...
  override def buildScan(requiredColumns: Array[String], filters: Array[Filter]): RDD[Row] = {
    ...
    new KuduRDD(context, batchSize, requiredColumns, predicates,
