...
class KuduContext(kuduMaster: String) extends Serializable {
  ...
  private[kudu] def writeRows(data: DataFrame, tableName: String, operation: OperationType) {
    ...
    data.foreachPartition(iterator => {
      ...
      val pendingErrors = writePartitionRows(new InterruptibleIterator[Row](taskContext,iterator),
                 schema, tableName, operation, taskContext)
  ...
  private def writePartitionRows(rows: Iterator[Row],
                                 ...
                                 taskContext: TaskContext): RowErrorsAndOverflowStatus = {
    ...
    val recordsWritten = taskContext.taskMetrics.outputMetrics.recordsWritten + rowSize
    KuduUtil.invokeMethod(taskContext.taskMetrics.outputMetrics, "setRecordsWritten", recordsWritten)
