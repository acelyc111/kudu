...
TEST_F(ReplicatedAlterTableTest, AlterReplicationFactor) {
  ...
  ASSERT_TRUE(s.IsInvalidArgument());
  ASSERT_STR_CONTAINS(s.ToString(), "illegal replication factor -1"
  ...
  ASSERT_TRUE(s.IsInvalidArgument());
  ASSERT_STR_CONTAINS(s.ToString(), "illegal replication factor 2"
  ...
  ASSERT_TRUE(s.IsInvalidArgument());
  ASSERT_STR_CONTAINS(s.ToString(), "illegal replication factor 9"
                                    " (max replication factor is 7)");
  NO_FATALS(VerifyTabletReplicaCount(3));
  ASSERT_EQ(2, tablet_replica_->tablet()->metadata()->schema_version());
