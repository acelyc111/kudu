  ...
  Status GenerateJsonReader(string content, unique_ptr<JsonReaderBase>* result) {
    ...
    switch (type) {
      case kString:
        ...
        return Status::OK();
      case kFile: {
        string fname = GetTestPath("JsonFileReaderTest.json");
        unique_ptr<WritableFile> writable_file;
        RETURN_NOT_OK(Env::Default()->NewWritableFile(fname, &writable_file));
        RETURN_NOT_OK(writable_file->Append(Slice(content)));
        RETURN_NOT_OK(writable_file->Close());
        result->reset(new JsonFileReader(fname));
        return Status::OK();
      }
      default:
        return Status::InvalidArgument(Substitute("Invalid type $0", type));
    }
  }
};

INSTANTIATE_TEST_CASE_P(JsonReader, JsonReaderTest, ::testing::Values(kString, kFile));
