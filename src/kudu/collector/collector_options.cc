...
DEFINE_string(collector_master_addrs, "127.0.0.1:7051",
              "Comma separated addresses of the masters which the "
              "collector should connect to. The masters do not "
              "read this flag -- configure the masters separately "
              "using 'rpc_bind_addresses'.");
TAG_FLAG(collector_master_addrs, stable);
...
CollectorOptions::CollectorOptions() {
  Status s = HostPort::ParseStrings(FLAGS_collector_master_addrs,
                                    ...
                                    &master_addresses);
  if (!s.ok()) {
    LOG(FATAL) << "Couldn't parse collector_master_addrs flag: " << s.ToString();
  }
}
