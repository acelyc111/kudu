  ...
  static Status ApplyOperateToSession(KuduSession* session,
    ...
    switch (type) {
      case KuduWriteOperation::Type::INSERT:
        ...
        break;
      case KuduWriteOperation::Type::INSERT_IGNORE:
        op.reset(table->NewInsertIgnore());
        break;
      case KuduWriteOperation::Type::UPDATE:
        op.reset(table->NewUpdate());
        break;
      case KuduWriteOperation::Type::UPDATE_IGNORE:
        op.reset(table->NewUpdateIgnore());
        break;
      case KuduWriteOperation::Type::UPSERT:
        op.reset(table->NewUpsert());
        break;
      case KuduWriteOperation::Type::UPSERT_IGNORE:
        op.reset(table->NewUpsertIgnore());
        break;
      default:
        CHECK(false);
    }
    RETURN_NOT_OK(op->mutable_row()->SetInt32("key", row_key));
    if (int_val) {
    ...
    if (immutable_val) {
      RETURN_NOT_OK(op->mutable_row()->SetInt32("immutable_val", immutable_val.get()));
    }
    if (immutable_val_with_default) {
      RETURN_NOT_OK(op->mutable_row()->SetInt32("immutable_val_with_default",
                                                    immutable_val_with_default.get()));
    }
    return session->Apply(op.release());
  }
...
TEST_F(ImmutableColumnTest, TestUpdateIgnore) {
  ...
  {
    // UPDATE_IGNORE the non-immutable columns.
    ASSERT_OK(ApplyOperateToSession(session.get(), client_table_,
                                    KuduWriteOperation::Type::UPDATE_IGNORE,
                                    1, 4, "update row", 6));
    DoTestVerifyRow(client_table_, 1, 4, "update row", 6, 4, 54321);
...
TEST_F(ImmutableColumnTest, TestUpsert) {
...

  {
    ...
    Status s = ApplyOperateToSession(session.get(), client_table_,
                                     KuduWriteOperation::Type::UPSERT,
                                     1, 4, "update row", 6, boost::none, 999);
    ASSERT_TRUE(s.IsIOError()) << s.ToString();
    ASSERT_EQ(1, session->CountPendingErrors());
    vector<KuduError*> errors;
    bool overflowed = false;
    session->GetPendingErrors(&errors, &overflowed);
    ASSERT_EQ(1, errors.size());
    ASSERT_TRUE(errors[0]->status().IsImmutable()) << errors[0]->status().ToString();
    DoTestVerifyRow(client_table_, 1, 2, "origin row", 3, 4, 54321);
  }
...
TEST_F(ImmutableColumnTest, TestUpsertIgnore) {
  ...
  {
    // UPSERT_IGNORE the immutable column with default value.
    ASSERT_OK(ApplyOperateToSession(session.get(), client_table_,
                                   KuduWriteOperation::Type::UPSERT_IGNORE,
                                   1, 6, "update row2", 8, boost::none, 999));
